...
test in this suite makes a real network call to Razorpay or Anthropic. The `anthropic`
dependency floor is pinned to `>=1.0,<2.0` to match what's actually installed and tested.

**`react-router-dom`** is pinned to the latest v6 (`6.28.0`); a moderate `npm audit` advisory
on that line (an open-redirect vector via a backslash in `<Link>`/`useNavigate` targets, plus
an SSR-only hydration issue that doesn't apply here — this is a pure client-rendered SPA) is
fixed only by a major-version jump to `7.18.3+`, which changes the router's API surface.
Checked this app's actual exposure: every `<Link to=...>` call in `frontend/src` builds its
target from a server-generated `RecoveryCase.id` (a UUID), never from raw user/customer
input — there is no code path where an attacker controls a navigation target, and no
`useNavigate()` calls exist anywhere in the app.
